...
OPERATORS = {
    "operator!=": '__ne__',       # was '__neq__' (typo in original)
    "operator==": '__eq__',
    "operator<": '__lt__',
    "operator<=": '__le__',
    "operator>": '__gt__',
    "operator>=": '__ge__',
    "operator bool": '__bool__',  # was '__nonzero__' (py2)
...
ARRAY_TYPES = {
    ...
    'Vt.BoolArray': 'bool',
    'Vt.FloatArray': 'float',
    'Vt.Int64Array': 'int',  # was 'long' (py2)
    'Vt.IntArray': 'int',
...
class SourceInfo:
    ...
    def get_valid_modules(self):
        ...
        Scans the pxr source tree for directories that contain wrap*.cpp files,
        which indicate a module with python bindings.

        (Replaces the old generateDocs._getModules() approach that was removed
        from the USD build system.)
        """
        if self._valid_modules is None:
            pxr_dir = os.path.join(self.srcdir, 'pxr')
            if not os.path.isdir(pxr_dir):
                raise RuntimeError("Cannot find pxr source directory: %s"
                                   % pxr_dir)
            module_names = []
            # Walk into each area (base, usd, imaging, usdImaging, etc.)
            for area in sorted(os.listdir(pxr_dir)):
                area_path = os.path.join(pxr_dir, area)
                if not os.path.isdir(area_path):
                    continue
                # Each subdirectory of an area is a potential module
                for lib in sorted(os.listdir(area_path)):
                    lib_path = os.path.join(area_path, lib)
                    if not os.path.isdir(lib_path):
                        continue
                    # A module has python bindings if it contains wrap*.cpp
                    has_wrap = any(
                        f.startswith('wrap') and f.endswith('.cpp')
                        for f in os.listdir(lib_path)
                        if os.path.isfile(os.path.join(lib_path, f))
                    )
                    if has_wrap:
                        module_names.append(capitalize(lib))

            # Sort longest-first so split_module matches the most specific
            # module name first (same behaviour as the original).
            self._valid_modules = sorted(module_names, reverse=True)
        ...
        if self._implicitly_convertible_types is None:
            output = subprocess.check_output(
                ['grep', 'implicitly_convertible', '-r',
                 ...
                 '--include=wrap*.cpp'],
                text=True)   # py3: decode to str directly
    ...
    def convert_typestr(self, node, is_arg=True):
        ...
        """
        if isinstance(node, str):       # was basestring
            typestr = node
        else:
            if node is None:
                return None
        ...
        typestr = ''.join(parts)
        typestr = typestr.replace(',', ', ')
        typestr = typestr.replace('&', '')  # FIXME: leftover from std::function
        typestr = typestr.replace('::', '.')
...
def convert_xml(path, srcdir, verbose=False, modules=None):
    ...
    def is_verbose(cpp_name):
        if isinstance(verbose, str):    # was basestring
...
def merge_dict(d1, d2):
    ...
    """
    for key, from_val in d1.items():                  # was .iteritems()
        # print key, from_val
        if key in d2:
            to_val = d2[key]
            if isinstance(from_val, dict) and isinstance(to_val, dict):
                merge_dict(from_val, to_val)           # was hasattr(.., 'iteritems')
            else:
                d2[key] = from_val
        else:
            d2[key] = from_val
...
def main(argv=None):
    ...
    if args.release:
        ...
        shutil.copytree(outputdir, pkgdir, dirs_exist_ok=True)  # was distutils.copy_tree
